...
test('warehouse request actions keep fallback pure and reconcile exact summaries from warehouse request snapshots', () => {
  const page = readFileSync('pages/warehouse-export-requests.vue', 'utf8')
  const helper = readFileSync('utils/fallbackOrderPatch.ts', 'utf8')

  assert.ok(page.includes('async function reconcileOrderSummary('))
  assert.ok(page.includes('requestsAfterTransition(row, patch)'))
  assert.ok(page.includes('orderWarehouseFulfillmentSummaryFromRequests('))
  assert.ok(page.includes("where('order_id', '==', orderId)"))
  assert.ok(page.includes('attempt <= 3'))
  assert.ok(page.includes('MAX_WAREHOUSE_RELEASE_LINES = 10'))
  assert.ok(page.includes("doc(db, 'warehouse_operations', operationId)"))
  assert.ok(page.includes("doc(db, 'notifications', notificationId)"))
  assert.ok(!page.includes("doc(collection(db, 'notifications'))"))
  assert.ok(page.includes("}, 'release')"))
  assert.ok(page.includes("}, 'cancel_release')"))
  assert.ok(!page.includes('loadPersistedOrder(orderId)'))
  assert.ok(!page.includes('loadScopedExportRequests([order], true)'))
  assert.ok(!page.includes('orderSummaryPatch:'))
