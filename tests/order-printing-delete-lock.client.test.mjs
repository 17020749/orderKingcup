...
test('trang đơn tải print_orders theo order_id và kiểm tra lại ngay trước xóa', () => {
  ...
  assert.match(page, /const localOrder = \{\s*\.\.\.\(editing\.value \|\| \{\}\),\s*\.\.\.result\.orderPayload,/)
  assert.doesNotMatch(page, /const localOrder = \{\s*\.\.\.\(editing\.value \|\| \{\}\),\s*\.\.\.baseOrder,\s*\.\.\.paymentSummary,/)
