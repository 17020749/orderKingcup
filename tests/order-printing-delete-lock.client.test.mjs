...
test('trang đơn tải print_orders theo order_id và kiểm tra lại ngay trước xóa', () => {
  ...
  assert.match(page, /const orderPayload = \{[\s\S]*printing_progress_count: 0[\s\S]*relation_lock_version: 1/)
...
test('Rules ưu tiên nhánh rẻ để không vượt giới hạn biểu thức', () => {
  ...
  assert.match(rules, /allow update: if changedAny\(\['deleted', 'active', 'deleted_at'\]\)\s*\? \(exportSoftDeleteAllowed\(docId\) \|\| exportOrderCascadeDeleteAllowed\(\)\)[\s\S]*?\? exportRequestReleaseAllowed\(docId\)/)
})
