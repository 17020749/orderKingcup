...
const filtered = computed(() => rows.value.filter(row =>
  normalizeText(`${row.request_id} ${row.order_code} ${row.customer_name} ${row.status} ${row.requested_by}`)
...
async function loadRows(force = false) {
  ...
  try {
    ...
    showToast(reportFirebaseError(error, 'Không tải được phiếu xuất kho.'), 'error')
  } finally {
    loading.value = false
  }
}
...
function applyLocalOrderSummary(orderId: string) {
  ...
  const progress = buildFulfillmentRows(itemsByOrder.value[orderId] || [], orderRequests)
...
function prepareLines() {
  ...
  const current = editing.value ? String(editing.value.request_id || editing.value.id) : ''
  const progress = buildFulfillmentRows(itemsByOrder.value[order.id] || [], requestsForOrder(order.id), current)
  ...
  exportLines.value = progress.map(row => ({
    ...row,
    export_quantity: oldQuantities.get(`${row.product_code}|${row.logo}`.toUpperCase()) || 0
  }))
...
function openModal(request?: any) {
  if (!hasPermission('orders.warehouse_export') && !hasPermission('*')) {
    return showToast('Bạn không có quyền tạo hoặc sửa phiếu xuất kho.', 'error')
  }
  if (request && !['cho_xu_ly', 'dang_xu_ly', 'da_tiep_nhan'].includes(String(request.status))) {
    return showToast('Phiếu đã được Warehouse xử lý xong nên không thể sửa.', 'error')
  }
...
async function saveRequest() {
  if (!selectedOrder.value) return showToast('Vui lòng chọn đơn hàng.', 'error')
  const chosen = validLines()
  if (!chosen.length) return showToast('Vui lòng nhập ít nhất một sản phẩm có số lượng xuất lớn hơn 0.', 'error')
  const invalid = chosen.find(line => toNumber(line.export_quantity) > line.available_to_request_qty)
  if (invalid) {
    return showToast(
  ...
  await withLoading(async () => {
    ...
    const previousTimeline = safeJsonParse(editing.value?.request_timeline_json, [])
    const timeline = [...(Array.isArray(previousTimeline) ? previousTimeline : []), {
      action: editing.value ? 'update' : 'create',
      title: editing.value ? 'Kingcup sửa yêu cầu xuất kho' : 'Kingcup tạo yêu cầu xuất kho',
    ...
    const payload = {
      ...
      requested_by_name: appUser.value?.display_name || appUser.value?.email || '',
    ...
    batch.set(doc(db, 'order_export_requests', form.id), {
      ...record,
    ...
    showToast(
      editing.value ? 'Đã cập nhật phiếu xuất kho.' : 'Đã gửi yêu cầu xuất kho sang Warehouse.',
      'success'
    )
  }).catch(error => {
    showToast(
      reportFirebaseError(error, editing.value ? 'Không cập nhật được phiếu xuất kho.' : 'Không tạo được phiếu xuất kho.'),
      'error'
    )
  }).finally(() => {
    saving.value = false
...
function canDeleteRequest(row: any) {
  return canDeletePermission.value
...
async function removeRequest(row: any) {
  ...
  await withLoading(async () => {
    ...
    const nextSummary = orderSummary(
      buildFulfillmentRows(itemsByOrder.value[row.order_id] || [], remainingRequests),
      remainingRequests
    )
    ...
    batch.set(doc(collection(db, 'activity_logs')), {
      ...
      changed_by: appUser.value?.email || '',
      after_json: JSON.stringify({ id: row.id, request_id: row.request_id, deleted: true }),
  ...
  }).catch(error => showToast(reportFirebaseError(error, 'Không xóa được phiếu xuất kho.'), 'error'))
}
...
function detailRequestLines(row: any) {
  ...
  return requestLineProgress(row).map((item: any) => {
    const previous = previousRows.find(line =>
      String(line.product_code || '').trim().toUpperCase() === String(item.product_code || '').trim().toUpperCase()
      && String(line.logo || '').trim().toUpperCase() === String(item.logo || '').trim().toUpperCase()
    )
    const ordered = previous?.ordered_qty || toNumber(item.order_quantity)
    const totalRequested = Math.min(ordered, toNumber(previous?.requested_qty) + toNumber(item.requested_qty))
    const totalProcessed = Math.min(totalRequested, toNumber(previous?.processed_qty) + toNumber(item.processed_qty))
    const totalExported = Math.min(totalProcessed || totalRequested, toNumber(previous?.exported_qty) + toNumber(item.exported_qty))
    return {
      ...item,
      ordered_qty: ordered,
...
<template>
  <AppShell>
    <PageHeader title="Phiếu xuất kho" subtitle="Yêu cầu xuất kho và trạng thái xử lý Warehouse">
      <button v-if="hasPermission('orders.warehouse_export') || hasPermission('*')" class="btn primary" @click="openModal()">+ Tạo yêu cầu xuất</button>
    </PageHeader>

    <div class="summary-grid">
      <div class="summary-card"><label>Tổng phiếu</label><strong>{{ summary.total }}</strong></div>
      <div class="summary-card"><label>Chờ Warehouse</label><strong>{{ summary.waiting }}</strong></div>
      <div class="summary-card"><label>Đã tiếp nhận</label><strong>{{ summary.accepted }}</strong></div>
      <div class="summary-card"><label>Đã xuất kho</label><strong>{{ summary.exported }}</strong></div>
    </div>

    <div class="card">
      <div class="toolbar">
        <input v-model="search" class="input" style="max-width:480px" placeholder="Tìm phiếu, mã đơn, khách hàng..." />
      ...
      <div v-else class="table-wrap">
        <table>
          <thead>
            <tr><th>Mã YC</th><th>Đơn hàng</th><th>Ngày yêu cầu</th><th>Người yêu cầu</th><th>Trạng thái</th><th>Phiếu kho</th><th>Thao tác</th></tr>
          </thead>
          <tbody>
            <tr v-for="row in filtered" :key="row.id">
              <td><b>{{ row.request_id }}</b></td>
              <td>{{ row.order_code }}<div class="small subtle">{{ row.customer_name }}</div></td>
              <td>{{ formatDateTime(row.requested_at) }}</td>
              <td>{{ row.requested_by }}</td>
              <td><span class="badge yellow">{{ statusLabel(row.status) }}</span></td>
              <td>{{ row.warehouse_export_code || '-' }}</td>
              <td>
                <div class="action-buttons">
                  <button class="btn-sm" @click="openDetail(row)">Xem</button>
                  <button
                    v-if="(hasPermission('orders.warehouse_export') || hasPermission('*')) && ['cho_xu_ly','dang_xu_ly','da_tiep_nhan'].includes(row.status)"
                    class="btn-sm"
                    @click="openModal(row)"
                  >Sửa</button>
                  <button v-if="canDeleteRequest(row)" class="btn-sm btn-delete" @click="removeRequest(row)">Xóa</button>
                  <button v-else-if="canDeletePermission" class="btn-sm" disabled>{{ row.status === 'da_xuat' ? 'Đã xuất' : 'Khóa' }}</button>
                </div>
              </td>
            </tr>
            <tr v-if="!filtered.length"><td colspan="7" class="empty">Không có phiếu xuất kho phù hợp.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <BaseModal v-if="showModal" :title="editing ? `Sửa phiếu ${form.request_id}` : 'Tạo yêu cầu xuất kho'" size="xl" :loading="saving" save-label="Lưu phiếu xuất" @close="showModal=false" @save="saveRequest">
      ...
      <div v-if="selectedOrder" class="detail-grid">
        ...
        <div class="detail-item"><label>Trạng thái đơn</label><strong>{{ selectedOrder.warehouse_fulfillment_status || 'Chưa xuất' }}</strong></div>
      </div>
      <div class="table-wrap" style="margin-top:16px">
        <table>
          <thead><tr><th>Sản phẩm</th><th>Logo</th><th>SL đơn</th><th>Đã yêu cầu</th><th>Đã xử lý</th><th>Đã xuất</th><th>Còn có thể yêu cầu</th><th>SL yêu cầu mới</th></tr></thead>
          <tbody><tr v-for="line in exportLines" :key="`${line.product_code}|${line.logo}`"><td>{{ line.product_name }}<div class="small subtle">{{ line.product_code }} · {{ line.unit }}</div></td><td>{{ line.logo || '-' }}</td><td>{{ line.ordered_qty }}</td><td>{{ line.requested_qty }}</td><td>{{ line.processed_qty }}</td><td>{{ line.exported_qty }}</td><td><b>{{ line.available_to_request_qty }}</b></td><td><input v-model.number="line.export_quantity" class="input" type="number" min="0" :max="line.available_to_request_qty" /></td></tr></tbody>
        </table>
      </div>
      <div class="form-group" style="margin-top:12px"><label>Ghi chú gửi Warehouse</label><textarea v-model="form.note" class="textarea" rows="3" /></div>
    </BaseModal>

    <BaseModal v-if="showDetailModal && selectedRequest" title="Chi tiết phiếu xuất kho" size="xl" :show-footer="false" @close="showDetailModal=false">
      <div class="detail-grid">
        <div class="detail-item"><label>ID Firestore</label><strong>{{ selectedRequest.id }}</strong></div>
        <div class="detail-item"><label>Mã phiếu</label><strong>{{ selectedRequest.request_id }}</strong></div>
        <div class="detail-item"><label>Đơn hàng</label><strong>{{ selectedRequest.order_code }} · {{ selectedRequest.order_id }}</strong></div>
        ...
        <div class="detail-item"><label>Người tạo phiếu</label><strong>{{ selectedRequest.requested_by || '-' }}</strong></div>
        <div class="detail-item"><label>Ngày giờ tạo</label><strong>{{ formatDateTime(selectedRequest.requested_at || selectedRequest.created_at) }}</strong></div>
        <div class="detail-item"><label>Cập nhật cuối</label><strong>{{ formatDateTime(selectedRequest.updated_at) }}</strong></div>
        <div class="detail-item"><label>Ngày xuất dự kiến</label><strong>{{ formatDateTime(selectedRequest.export_date) }}</strong></div>
        <div class="detail-item"><label>Mã phiếu Warehouse</label><strong>{{ selectedRequest.warehouse_export_code || '-' }}</strong></div>
        <div class="detail-item"><label>Warehouse xử lý</label><strong>{{ selectedRequest.warehouse_handled_by || '-' }}</strong></div>
        <div class="detail-item"><label>Ngày Warehouse xử lý</label><strong>{{ formatDateTime(selectedRequest.warehouse_handled_at) || '-' }}</strong></div>
        <div class="detail-item"><label>Ghi chú Warehouse</label><strong>{{ selectedRequest.warehouse_note || '-' }}</strong></div>
        <div class="detail-item"><label>Ghi chú yêu cầu</label><strong>{{ safeJsonParse(selectedRequest.payload_json, {}).note || '-' }}</strong></div>
      </div>
      <h3>Sản phẩm và tiến độ xuất</h3>
      <div class="table-wrap"><table style="min-width:1050px"><thead><tr><th>Sản phẩm</th><th>Logo</th><th>SL đơn</th><th>SL phiếu này</th><th>Đã yêu cầu trước</th><th>Đã xử lý</th><th>Đã xuất</th><th>Còn có thể yêu cầu</th><th>Còn phải xuất</th></tr></thead><tbody><tr v-for="(item,index) in detailRequestLines(selectedRequest)" :key="index"><td><b>{{ item.product_name }}</b><div class="small subtle">{{ item.product_code }} · {{ item.unit }}</div></td><td>{{ item.logo || '-' }}</td><td>{{ item.ordered_qty }}</td><td>{{ item.requested_qty }}</td><td>{{ item.requested_before_qty }}</td><td>{{ item.processed_total_qty }}</td><td>{{ item.exported_total_qty }}</td><td><b>{{ item.available_after_qty }}</b></td><td>{{ item.remaining_after_qty }}</td></tr></tbody></table></div>
