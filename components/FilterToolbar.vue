  ...
  <div class="toolbar filter-toolbar">
    ...
    <template v-for="filter in filters" :key="filter.key">
      <input
        ...
        :style="{ maxWidth: filter.width || '170px' }"
        @input="emit('update:filter', filter.key, ($event.target as HTMLInputElement).value)"
      />
      <select
        v-else
        :value="filterValue(filter.key)"
      ...
      >
        ...
        <option v-for="option in filter.options || []" :key="`${filter.key}-${option.value}`" :value="option.value">
          {{ option.label }}
        </option>
      </select>
    </template>
    ...
    <button v-if="showRefresh" class="btn" type="button" :disabled="loading" @click="emit('refresh')">
      {{ loading ? 'Đang tải...' : refreshLabel }}
    </button>
