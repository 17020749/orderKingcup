...
test('dashboard keeps the accurate client aggregation path until a backend summary exists', () => {
  ...
  const dashboardSnapshot = source('composables/useDashboardSnapshot.ts')
  const snapshotBuilder = source('utils/dashboardSnapshot.mjs')
  ...
  assert.doesNotMatch(dashboardSnapshot, /dashboard_summaries/)
