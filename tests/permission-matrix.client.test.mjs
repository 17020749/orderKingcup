...
test('customers.view_all chỉ mở rộng phạm vi khách hàng và không tự cấp action', () => {
  const grants = grantSet('customers.view_all')
  for (const required of ['customers.view_all', 'customers.view', 'page.customers']) {
    assert.equal(grants.has(required), true, `Thiếu ${required}`)
  }
  for (const action of ['customers.create', 'customers.edit', 'customers.delete', 'orders.view_all']) {
    assert.equal(grants.has(action), false, `Không được tự cấp ${action}`)
  }
})
