...
async function save() {
  ...
  const plate = String(form.vehicle_plate || '').trim().toUpperCase()
  const driver = String(form.driver_name || '').trim()
  if (!name) return showToast('Vui lòng nhập tên nhà xe.', 'error')
  if (!plate) return showToast('Vui lòng nhập biển số xe.', 'error')
  const duplicated = rows.value.some(row => row.id !== editing.value?.id && normalizeText(row.vehicle_plate || '') === normalizeText(plate))
  if (duplicated) return showToast('Biển số xe này đã tồn tại.', 'error')

  saving.value = true
  await withLoading(async () => {
    const actor = appUser.value?.email || ''
    if (editing.value) {
      await updateDoc(doc(db, 'transport_carriers', editing.value.id), {
        carrier_name: name,
        carrier_phone: phone,
        vehicle_plate: plate,
        driver_name: driver,
...
<template>
  <AppShell>
    <PageHeader title="Danh sách nhà xe" subtitle="Quản lý thông tin nhà xe dùng chung cho các nghiệp vụ vận chuyển">
    ...
    <div class="card" style="margin:24px">
      <div class="filter-bar">
        <input v-model="search" class="input" placeholder="Tìm tên nhà xe, số điện thoại, biển số, tài xế..." />
...
<thead><tr><th>Mã</th><th>Tên nhà xe</th><th>Số điện thoại</th><th>Biển số xe</th><th>Chủ xe/Tài xế</th><th>Cập nhật</th><th>Thao tác</th></tr></thead>
<tbody>
  <tr v-for="row in filtered" :key="row.id">
    <td><b>{{ row.carrier_code || '-' }}</b></td>
    <td>{{ row.carrier_name || '-' }}</td>
    <td>{{ row.carrier_phone || '-' }}</td>
    <td><b>{{ row.vehicle_plate || '-' }}</b></td>
  ...
  <tr v-if="!filtered.length"><td colspan="7" class="empty">Chưa có nhà xe phù hợp.</td></tr>
</tbody>
        </table>
      </div>
    </div>
    ...
    <BaseModal v-if="showModal" :title="editing ? 'Sửa nhà xe' : 'Thêm nhà xe'" size="lg" :loading="saving" :save-label="editing ? 'Lưu thay đổi' : 'Thêm nhà xe'" @close="showModal=false" @save="save">
      <div class="form-grid">
        ...
        <div class="form-group"><label>Biển số xe <span class="required">*</span></label><input v-model="form.vehicle_plate" class="input" /></div>
        <div class="form-group full"><label>Tên chủ xe/Tài xế</label><input v-model="form.driver_name" class="input" /></div>
    ...
    <BaseModal v-if="showDetailModal && selectedDetail" title="Chi tiết nhà xe" size="lg" :show-footer="false" @close="showDetailModal=false">
      <div class="detail-grid">
        ...
        <div class="detail-item"><label>Biển số xe</label><strong>{{ selectedDetail.vehicle_plate || '-' }}</strong></div>
...
.filter-bar .input { max-width:620px; }
