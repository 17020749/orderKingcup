...
import type { BusTransportDoc, CustomerDoc, OrderDoc } from '~/types/models'
...
const receiverName = computed(() => String(
  ...
  || props.busTransport?.receiver_name
  || props.order?.customer_name
  || props.request?.customer_name
  || '',
).trim())
const receiverPhone = computed(() => String(
  props.customer?.phone
  || props.busTransport?.receiver_phone
  || props.order?.phone
  || '',
).trim())
const receiverAddress = computed(() => String(
  props.customer?.shipping_address
  || props.customer?.billing_address
  || props.busTransport?.receiver_address
  || '',
).trim())
const orderCode = computed(() => String(
  props.order?.order_code
  || props.busTransport?.order_code
  || props.request?.order_code
  || props.sourceCode
  || '',
).trim())
    ...
    <div class="table-wrap" style="margin-top:14px">
      <table style="min-width:720px">
        ...
        <tbody>
          ...
          <tr v-if="!printRows.length"><td colspan="4" class="empty">Yêu cầu chưa có sản phẩm để in.</td></tr>
        </tbody>
      </table>
    </div>
...
.receiver-box label,
.receiver-box strong { display: block; }
.label-summary label,
.receiver-box label { margin-bottom: 3px; color: #64748b; font-size: 12px; }
...
@media (max-width: 560px) {
  .label-summary,
  .receiver-box { grid-template-columns: 1fr; }
  .receiver-box .full { grid-column: auto; }
